...
// idempotent. Facts left legacy_unverified stay eligible for a later pass, so
// a pass walks fact ids with a cursor rather than re-reading the same first
// page. No bulk run happens during deployment.
...
export type ProofBackfillReport = Readonly<{
  dry_run: boolean;
  examined: number;
  /** Pass as after_fact_id to continue; null once the pass is complete. */
  next_after_fact_id: string | null;
  revalidated: number;
...
export async function backfillFactPrecisionProofs(
  db: RowQueryExecutor,
  options: { dry_run: boolean; limit: number; after_fact_id: string | null; reader: RetainedSourceReader; validation_method: string },
): Promise<ProofBackfillReport> {
  assertNonEmptyString(options.validation_method, "validation_method");
  if (options.after_fact_id !== null) assertUuidV4(options.after_fact_id, "after_fact_id");
  ...
  const facts = (await db.query<BackfillFact & { has_proof: boolean }>(
    `select f.fact_id::text, f.source_id::text, m.metric_key, f.period_end::text, f.fiscal_year, f.fiscal_period,
       ...
       left join current_fact_precision_attestations latest on latest.fact_id = f.fact_id
      where f.method in ('reported', 'extracted')
        ...
        and ($2::uuid is null or f.fact_id > $2::uuid)
      order by f.fact_id
      limit $1`,
    [options.limit, options.after_fact_id],
  )).rows;

  const outcomes: ProofBackfillOutcome[] = [];
  for (const fact of facts) {
    const retained = await options.reader(fact);
    const check = retained === null ? "missing_source_bytes" : checkTokenAgainstStoredValue(retained.raw_token, fact.value_text).status;
  ...
  return {
    ...
    next_after_fact_id: facts.length === options.limit ? facts[facts.length - 1]!.fact_id : null,
