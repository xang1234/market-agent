...
export async function handleDiscoveryHttp(
  ...
  if (!url.pathname.startsWith("/v1/discovery")) return false;
...
async function dispatch(url: URL, req: IncomingMessage, input: { userId: string; service: DiscoveryService }): Promise<Dispatch> {
  ...
  if (campaign) {
    ...
    if (req.method === "DELETE") { await input.service.deleteCampaign(input.userId, campaignId); return NO_CONTENT; }
    return methodNotAllowed();
  }
  ...
  if (runs) {
    ...
    if (req.method === "POST") {
      ...
      return { status: 201, body: await input.service.startRun(input.userId, campaignId, {
        brief_version: integer(body.brief_version, "brief_version", 1), brief_hash: requireString(body.brief_hash, "brief_hash"), request_key: routeId(requireString(body.request_key, "request_key"), "request_key"),
      }) };
    }
    return methodNotAllowed();
  }
  ...
  if (cancel) return req.method === "POST" ? { status: 200, body: await input.service.cancelRun(input.userId, routeId(cancel[1], "run_id")) } : methodNotAllowed();
  return NOT_FOUND;
}
...
async function objectBody(req: IncomingMessage): Promise<Record<string, unknown>> {
  let text = "";
  for await (const chunk of req) text += Buffer.isBuffer(chunk) ? chunk.toString("utf8") : Buffer.from(chunk).toString("utf8");
  let value: unknown;
...
function routeId(value: string | undefined, label: string): string {
  if (value === undefined || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(decodeURIComponent(value))) throw new DiscoveryError("validation", `${label} must be a UUID`);
