...
const TRUSTED_PROXY_SECRET = "portfolio-auth-test-secret";
...
test("trusted-proxy auth rejects expired and tampered portfolio signatures before DB access", async (t) => {
  ...
  for (const signature of [tamperedTimestamp, expired]) {
    const response = await fetch(`${base}/v1/portfolios`, {
      headers: {
