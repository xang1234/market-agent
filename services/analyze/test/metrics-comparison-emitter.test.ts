...
const PEER: IssuerSubjectRef = { kind: "issuer", id: "33333333-3333-4333-a333-333333333333" };
const REV_PRIMARY = "f0000000-0000-4000-8000-000000000001";
const REV_PEER = "f0000000-0000-4000-8000-000000000002";

// A minimal key-stats envelope: gross_margin (computed) whose denominator is the
// revenue statement line (which the fetcher reuses as the revenue metric).
...
const resolver: PeerSetResolver = {
  async resolvePeers() {
    return [PEER];
  },
};
...
// Stateful fake DB: resolves derived metric_ids, captures fact inserts, and
// serves the fact-load select for both inserted (derived) and pre-seeded
// (reused revenue) facts.
function fakeDb() {
  const facts = new Map<string, Record<string, unknown>>();
  // Pre-seed the reused revenue facts the fetcher points at.
