...
// checkpoint and finalization write first re-checks the fence under the run
// row lock, so a worker whose lease expired, was superseded, or whose run was
// cancelled can never write. Discovery-owned runs are resumed only under the
// parent campaign's authority, never reclaimed by a generic worker.
...
export async function acquireLease(
  ...
  try {
    ...
    const result = await (async (): Promise<AcquireLeaseResult> => {
...
/**
 * Locks the run row and verifies the lease is still the current, live one.
 * Call inside the transaction whose writes the fence protects.
 */
export async function assertLeaseFence(
  ...
  if (run.cancel_requested_at !== null && !options.allowCancelRequested) throw new StaleLeaseError("cancel_requested");
  return run;
}
