...
import { acquireLease, assertLeaseFence, renewLease, StaleLeaseError, type RunLease } from "../src/lease.ts";
import { requestCancellation, reserveRun, RunTransitionError, transitionRun } from "../src/run-repo.ts";
import { declareUnits, listUnits, markUnitComputed, rejectUnit, UnitClosureConflictError } from "../src/unit-repo.ts";
import { authorityFor, engineDatabase, IDS, revenuePlan } from "./db-fixtures.ts";

async function inTransaction<T>(db: Client, action: () => Promise<T>): Promise<T> {
...
test("financial run leases, lifecycle, units, and events", { timeout: 240_000 }, async (t) => {
  ...
  await t.test("leases are exclusive, expire, and fence stale workers out with a higher epoch", async () => {
    ...
    await inTransaction(db, () => declareUnits(db, first.lease, plan));
    ...
    await assert.rejects(() => inTransaction(db, () => assertLeaseFence(db, first.lease)), StaleLeaseError);
    await assert.rejects(() => inTransaction(db, () => markUnitComputed(db, first.lease, "a_unit", "complete")), StaleLeaseError);
  ...
  await t.test("lifecycle transitions are validated, and terminal runs release their lease", async () => {
    ...
    await assert.rejects(() => inTransaction(db, () => transitionRun(db, lease, "completed", { coverage_state: "complete" })), RunTransitionError);
    await assert.rejects(() => inTransaction(db, () => transitionRun(db, lease, "failed")), /failure code/);
    await inTransaction(db, () => transitionRun(db, lease, "ready_to_seal"));
    await assert.rejects(() => inTransaction(db, () => transitionRun(db, lease, "completed")), /coverage/);
    const completed = await inTransaction(db, () => transitionRun(db, lease, "completed", { coverage_state: "partial" }));
    ...
    await assert.rejects(() => inTransaction(db, () => assertLeaseFence(db, lease)), /terminal/);
  });
  ...
  await t.test("cancellation wins over workers: at once without a lease, at the next fence with one", async () => {
    ...
    await assert.rejects(() => inTransaction(db, () => assertLeaseFence(db, lease)), /cancel_requested/);
    const final = await inTransaction(db, () => transitionRun(db, lease, "cancelled"));
  ...
  await t.test("units are declared once, progress is idempotent, and rejection is final", async () => {
    ...
    await inTransaction(db, () => declareUnits(db, lease, plan));
    await inTransaction(db, () => declareUnits(db, lease, plan));
    ...
    await assert.rejects(() => inTransaction(db, () => declareUnits(db, lease, narrowed)), UnitClosureConflictError);

    assert.equal(await inTransaction(db, () => markUnitComputed(db, lease, "a_unit", "complete")), true);
    assert.equal(await inTransaction(db, () => markUnitComputed(db, lease, "a_unit", "complete")), false);
    assert.equal(await inTransaction(db, () => rejectUnit(db, lease, "b_unit", "integrity_failure")), true);
    assert.equal(await inTransaction(db, () => markUnitComputed(db, lease, "b_unit", "complete")), false);
