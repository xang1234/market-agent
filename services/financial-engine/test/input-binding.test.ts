...
test("historical input binding", { timeout: 240_000 }, async (t) => {
  ...
  await t.test("binds the original public before the cutoff despite later ingestion, and persists it", async () => {
    ...
    const result = await bindPlanInputs({ client: db, run_id: runId, plan, authority, evidence: createEvidenceFinancialPort });
  ...
  await t.test("a retry reuses the persisted binding even after new evidence arrives", async () => {
    ...
    const runId = await insertPlanAndRun(db, plan, authority);
    const first = await bindPlanInputs({ client: db, run_id: runId, plan, authority, evidence: createEvidenceFinancialPort });
    ...
    const retry = await bindPlanInputs({ client: db, run_id: runId, plan, authority, evidence: createEvidenceFinancialPort });
  ...
  await t.test("the candidate-set digest changes when the authorized candidates change", async () => {
    const plan = revenuePlan();
    const before = await bindPlanInputs({ client: db, run_id: await insertPlanAndRun(db, plan, authority), plan, authority, evidence: createEvidenceFinancialPort });
    await db.query(`update facts set invalidated_at = now() where fact_id = $1`, [IDS.restated]);
    const afterPlan = revenuePlan();
    const after = await bindPlanInputs({ client: db, run_id: await insertPlanAndRun(db, afterPlan, authority), plan: afterPlan, authority, evidence: createEvidenceFinancialPort });
    await db.query(`update facts set invalidated_at = null where fact_id = $1`, [IDS.restated]);
  ...
  await t.test("another owner cannot bind a run, and a binding failure rolls back cleanly", async () => {
    const plan = revenuePlan();
    const runId = await insertPlanAndRun(db, plan, authority);
    await assert.rejects(
      () => bindPlanInputs({ client: db, run_id: runId, plan, authority: authorityFor(IDS.other), evidence: createEvidenceFinancialPort }),
      FinancialBindingError,
    );
    await assert.rejects(
      () => bindPlanInputs({ client: db, run_id: runId, plan: revenuePlan(), authority, evidence: createEvidenceFinancialPort }),
