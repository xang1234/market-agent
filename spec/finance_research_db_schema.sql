...
create table discovery_campaigns (
  ...
  archived_at timestamptz, draft_lock_token uuid, draft_lock_until timestamptz,
...
create table discovery_briefs (
  ...
  version integer not null check (version > 0), brief jsonb not null check (jsonb_typeof(brief) = 'object'),
  content_hash text not null check (content_hash ~ '^sha256:[0-9a-f]{64}$'), approved_at timestamptz,
  created_at timestamptz not null default now(), unique (campaign_id, version), unique (brief_id, campaign_id)
);
create function prevent_discovery_approved_brief_mutation() returns trigger language plpgsql as $$
begin
  if old.approved_at is not null and new is distinct from old then raise exception 'approved discovery briefs are immutable'; end if;
  return new;
end;
$$;
create trigger discovery_briefs_immutable before update on discovery_briefs
for each row execute function prevent_discovery_approved_brief_mutation();
create table discovery_runs (
  run_id uuid primary key default gen_random_uuid(), campaign_id uuid not null, user_id uuid not null, brief_id uuid not null, request_key uuid not null,
  status text not null check (status in ('queued','running','completed','partial','failed','cancelled')),
  stage text not null check (stage in ('queued','discovery','research','finalization')), policy_version text not null check (length(btrim(policy_version)) > 0),
  model_config jsonb not null default '[]'::jsonb check (jsonb_typeof(model_config) = 'array'), limits jsonb not null check (jsonb_typeof(limits) = 'object'),
  usage jsonb not null check (jsonb_typeof(usage) = 'object'), phase_usage jsonb not null default '{}'::jsonb check (jsonb_typeof(phase_usage) = 'object'),
  checkpoint jsonb not null check (jsonb_typeof(checkpoint) = 'object'), coverage jsonb not null check (jsonb_typeof(coverage) = 'object'),
  lease_owner text, lease_epoch bigint not null default 0 check (lease_epoch >= 0), lease_expires_at timestamptz,
  next_event_sequence bigint not null default 0 check (next_event_sequence >= 0), cancel_requested_at timestamptz, started_at timestamptz, finished_at timestamptz,
  created_at timestamptz not null default now(),
...
create table discovery_candidates (
  ...
  identity_display jsonb check (identity_display is null or jsonb_typeof(identity_display) = 'object'), origins jsonb not null check (jsonb_typeof(origins) = 'array'),
  mechanism_ids jsonb not null check (jsonb_typeof(mechanism_ids) = 'array'), lead_hit_ids jsonb not null check (jsonb_typeof(lead_hit_ids) = 'array'),
  reason_codes jsonb not null check (jsonb_typeof(reason_codes) = 'array'), first_seen jsonb not null check (jsonb_typeof(first_seen) = 'array' and jsonb_array_length(first_seen) = 2),
  seed boolean not null, primary_domain_lead boolean not null, name text not null check (length(btrim(name)) > 0),
...
create unique index discovery_shortlist_rank on discovery_candidates(run_id, rank) where rank is not null;
create table discovery_attempts (
  ...
  phase text not null check (phase in ('draft','discovery','research','verification')), candidate_id uuid references discovery_candidates(candidate_id) on delete cascade, model_initial boolean not null default false, model_role text check (model_role is null or model_role in ('analyst','skeptic')), reserved_worker_id text, reserved_lease_epoch bigint check (reserved_lease_epoch is null or reserved_lease_epoch >= 0),
  constraint discovery_attempts_initial_model_role check ((model_initial and model_role is not null) or (not model_initial and model_role is null)),
...
create table discovery_events (
  run_id uuid not null references discovery_runs(run_id) on delete cascade, sequence bigint not null check (sequence > 0),
...
create table analyze_run_financial_sections (
  ...
  -- A section is the sealed block itself; it goes with its certificate (0051).
...
-- Verified finance: erasure (0051). A deleted financial run takes its plan,
-- inputs, results, certificates, certificate snapshots, replays, and every
-- surface copy of its sealed block with it.
...
-- Memo sections cascade from their certificate (see analyze_run_financial_sections).
