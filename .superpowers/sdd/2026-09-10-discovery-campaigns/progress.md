...
Final re-review 2 fix: RED — real Ajv validation accepted numeric `0.0000001` (serialized as `1e-7`) and a 101-digit string under DiscoveryMetricCheck, while the shared `parseBrief` correctly rejected both. GREEN — public thresholds now use canonical exact decimal strings for every fractional value and safe JSON integers only for compatibility. The shared parser, OpenAPI component, and browser decoder agree on optional-minus/no-leading-zero decimal grammar, 100 total digits, 100 fractional places, no exponent/plus/whitespace, and safe integer range ±9007199254740991. Existing stored fractional-number thesis and brief records are normalized to canonical strings only while reading; new writes remain strict. Ajv semantic parity tests cover all five operators, 0.3, string 0.0000001, safe/unsafe numeric inputs, 100/101 digit boundaries, excess scale, and malformed forms; contract mutations fail. Dev API1/1, Agents24/24, Discovery assessment/validation/contracts23/23, OpenAPI12/12, focused web API/ThesisPanel15/15, web typecheck/build, and strict affected-source compile (with pre-existing library declaration checks skipped) pass. Feature false; human gate Pending.
