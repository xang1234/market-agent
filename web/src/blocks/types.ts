...
export const CERTIFIED_FINANCIAL_BLOCK_KINDS = ['financial_answer'] as const
export type CertifiedFinancialBlockKind = (typeof CERTIFIED_FINANCIAL_BLOCK_KINDS)[number]
...
// Certified financial answers. Every string below is generated by the
// snapshot verifier from the committed results it just checked; the renderer
// prints them and never formats or compares numbers itself. Mirrors
// `FinancialAnswerContent` in services/financial-core, whose contract test
// checks the generator's output against the block schema.
export const SUPPORTED_FINANCIAL_PRESENTATION_VERSION = 'financial-presentation.v1'

export type FinancialUnit =
  | { kind: 'currency' | 'currency_per_share'; currency: string }
  | { kind: 'ratio' | 'percent' | 'percentage_points' | 'basis_points' | 'shares' | 'count' }

export type FinancialPresentationLabel =
  | { kind: 'subject'; text: string; slot_id: string }
  | { kind: 'measure' | 'period'; text: string }

export type FinancialPresentedPayload =
  | { kind: 'value'; text: string; full_text: string; value: string; unit: FinancialUnit; exact: boolean }
  | { kind: 'predicate'; text: string; outcome: boolean }
  | {
      kind: 'ranking'
      text: string
      complete: boolean
      order: ReadonlyArray<{ subject_label_id: string; rank: number }>
      leader_label_ids: ReadonlyArray<string> | null
    }
  | { kind: 'gap'; text: string; reason_code: string }

export type FinancialPresentedResult = {
  result_id: string
  output_id: string
  disposition: string
  ...
  presented: FinancialPresentedPayload
  result_hash: string
}

export type FinancialTablePresentation = {
  kind: 'table'
  caption: string
  columns: ReadonlyArray<{ column_id: string; measure_label_id: string; period_label_id: string }>
  rows: ReadonlyArray<{ subject_label_id: string; cells: ReadonlyArray<string | null> }>
  ascending: Readonly<Record<string, ReadonlyArray<number>>>
}

export type FinancialSeriesPresentation = {
  kind: 'series'
  subject_label_id: string
  measure_label_id: string
  points: ReadonlyArray<{ period_label_id: string; result_id: string }>
}

export type FinancialPresentation =
  | { kind: 'scalar' | 'predicate' | 'gap'; result_id: string }
  | FinancialTablePresentation
  | FinancialSeriesPresentation

export type FinancialAnswerContent = {
  presentation_version: typeof SUPPORTED_FINANCIAL_PRESENTATION_VERSION
  template_version: string
  run_id: string
  unit_id: string
  knowledge_cutoff: string
  coverage: { state: 'complete' | 'partial' | 'none'; requested: number; verified: number }
  labels: Readonly<Record<string, FinancialPresentationLabel>>
  results: ReadonlyArray<FinancialPresentedResult>
  presentations: ReadonlyArray<FinancialPresentation>
}

export type FinancialAnswerBlock = Omit<BaseBlock, 'title'> & {
  kind: 'financial_answer'
  // No free-text title in the certified lane.
  title?: never
  presentation_hash: string
  // An unknown version is carried only so it can be shown as unsupported.
  financial: FinancialAnswerContent | { presentation_version: string }
}
