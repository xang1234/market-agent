...
export function CampaignList() {
  ...
  useEffect(() => {
    if (!userId) return;
    let active = true;
    listCampaigns({ userId }).then((page) => { if (active) { setCampaigns(page.items); setNextCursor(page.next_cursor); } }).catch((caught) => { if (active) setError(discoveryMessage(caught, "Campaigns could not be loaded.")); });
    return () => { active = false; };
  }, [userId]);
  ...
  async function loadMore() {
    ...
    setError(null);
    try {
      const page = await listCampaigns({ userId, cursor });
      setCampaigns((current) => [...current, ...page.items]);
      setNextCursor(page.next_cursor);
    } catch (caught) {
      setError(discoveryMessage(caught, "More campaigns could not be loaded."));
    } finally {
